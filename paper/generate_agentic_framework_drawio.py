...
def build_architecture_page(doc: DrawioDocument) -> None:
    ...
    orchestrator = add_card(
        ...
        [
            "- Canonical flow: build_pack -> explain -> audit -> patch_retrieve? -> ideate -> score -> blueprint -> publish",
    ...
    evaluation = add_card(
        ...
        [
            ...
            "- Orchestrator is one method among several baselines: single-shot LLM, retrieval-summary direct, heuristic bridge, pack-query baseline, random target control",
    ...
    metrics = add_card(
        ...
        [
            "- Recovery labels: gold_recovered, future_neighbor_only, historical_confound, not_recovered",
            "- Metrics: gold recall@k, gold MRR, cue-weighted variants, and idea-score summaries",
...
def build_evaluation_page(doc: DrawioDocument) -> None:
    ...
    split = add_card(
        ...
        [
            ...
            "- Current benchmark scores only the primary future window",
    ...
    targets = add_card(
        ...
        425,
        290,
        "3. Target pool + gold future assignment",
        [
            "- Gap targets from `backend.top_gaps()`",
            "- Cluster-pair targets from shared gaps first, then cluster combinations",
            "- Build focused evidence packs and score future papers against that target pool",
            "- Filter leakage and keep frontier-eligible gold future papers",
        ],
        "#FFFFFF",
        "#73A67E",
    )
    generation = add_card(
        ...
        "4. Generation sweep",
        [
            "- Nested loop over `methods x seeds x gold future papers`",
    ...
    matching = add_card(
        ...
        [
            ...
            "- Record gold-paper rank, best historical confounder, and best future neighbor",
            "- Classify the hypothesis into a recovery-style outcome label",
        ],
        "#FFFFFF",
        "#C49A63",
    )
    ...
    methods = add_card(
        ...
        [
            "- `orchestrator`: full evidence -> explain -> audit -> patch_retrieve -> ideate -> score -> blueprint path",
            "- `single_shot_llm`: one structured ideation call over the evidence pack",
            "- `retrieval_summary_direct`: summary first, then ideation",
            "- `heuristic_bridge`: deterministic bridge generator from pack fingerprints",
            "- `pack_query_baseline`: deterministic retrieval-oriented pack baseline",
            "- `random_target_control`: generate from a random historical target",
        ],
        "#FFFFFF",
        "#73A67E",
    )
    ...
    indexes = add_card(
        ...
        [
            ...
            "- Optional sensitivity split can be built for broader future analysis",
    ...
    classes = add_card(
        ...
        [
            ...
            "- Final labels: `gold_recovered`, `future_neighbor_only`, `historical_confound`, `not_recovered`",
            "- Metrics include gold recall@k, gold MRR, cue-weighted variants, and idea scores",
        ],
        "#FFFFFF",
        "#C49A63",
    )
